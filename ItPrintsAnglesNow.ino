...
void findState(float refAngleYAW ,float refAnglePITCH,float refAngleROLL, int *state, int *angle, int angleAccuracy)
{
  read_sensor_values();
  ...
  if (abs((int)currentAnglePITCH - refAnglePITCH) < angleAccuracy) 
  {
   ...
   state[0] = 0;
   angle[0] = (int) currentAnglePITCH - refAnglePITCH;
  }
...
if (abs((int)currentAngleYAW - refAngleYAW) < angleAccuracy)
  {
   ...
   state[1] = 0;
   angle[1] = (int) currentAngleYAW - refAngleYAW;
  }
...
//  stop_at_angle(ROLL, referenceAngles[ROLL], 20);
  ...
  while(1)
  {
    findState(referenceAngles[YAW],referenceAngles[PITCH],referenceAngles[ROLL],statePointer,anglePointer, 5);
    lcd.setCursor(0,0);
    lcd.clear();
    switch (state[0])
    {
